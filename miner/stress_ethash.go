...
// This file contains a miner stress test based on the Sevash consensus engine.
package main
...
func main() {
	...
	for _, node := range nodes {
		...
		if err := severeum.StartMining(1); err != nil {
			panic(err)
		}
	}
	time.Sleep(3 * time.Second)
	...
	for {
		...
		if err := severeum.TxPool().AddLocal(tx); err != nil {
			panic(err)
		}
		nonces[index]++

		// Wait if we're too saturated
		if pend, _ := severeum.TxPool().Stats(); pend > 2048 {
			time.Sleep(100 * time.Millisecond)
		}
	}
}

// makeGenesis creates a custom Sevash genesis block based on some pre-defined
// faucet accounts.
...
func makeMiner(genesis *core.Genesis) (*node.Node, error) {
	// Define the basic configurations for the Severeum node
	...
	// Start the node and configure a full Severeum node on it

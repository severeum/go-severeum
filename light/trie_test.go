...
func TestNodeIterator(t *testing.T) {
	var (
		fulldb  = sevdb.NewMemDatabase()
		lightdb = sevdb.NewMemDatabase()
		gspec   = core.Genesis{Alloc: core.GenesisAlloc{testBankAddress: {Balance: testBankFunds}}}
		genesis = gspec.MustCommit(fulldb)
	)
	gspec.MustCommit(lightdb)
	blockchain, _ := core.NewBlockChain(fulldb, nil, params.TestChainConfig, sevash.NewFullFaker(), vm.Config{}, nil)
	gchain, _ := core.GenerateChain(params.TestChainConfig, genesis, sevash.NewFaker(), fulldb, 4, testChainGen)

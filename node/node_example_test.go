...
//
// The following msevods are needed to implement a node.Service:
//  - Protocols() []p2p.Protocol - devp2p protocols the service can communicate on
//  - APIs() []rpc.API           - api msevods the service wants to expose on rpc channels
//  - Start() error              - msevod invoked when the node is ready to start the service
//  - Stop() error               - msevod invoked when the node terminates the service
...
func ExampleService() {
	...
	// the factory msevod approach is to support service restarts without relying on the

...
// NewPublicSevereumAPI creates a new Severeum protocol API.
func NewPublicSevereumAPI(b Backend) *PublicSevereumAPI {
	return &PublicSevereumAPI{b}
}

// GasPrice returns a suggestion for a gas price.
func (s *PublicSevereumAPI) GasPrice(ctx context.Context) (*hexutil.Big, error) {
	price, err := s.b.SuggestPrice(ctx)
	return (*hexutil.Big)(price), err
}

// ProtocolVersion returns the current Severeum protocol version this node supports
func (s *PublicSevereumAPI) ProtocolVersion() hexutil.Uint {
...
func (s *PublicSevereumAPI) Syncing() (interface{}, error) {
...
//   keccak256("\x19Severeum Signed Message:\n"${message length}${message}).
//
// This gives context to the signed message and prevents signing of transactions.
func signHash(data []byte) []byte {
	msg := fmt.Sprintf("\x19Severeum Signed Message:\n%d%s", len(data), data)
	return crypto.Keccak256([]byte(msg))
}

// Sign calculates an Severeum ECDSA signature for:
// keccack256("\x19Severeum Signed Message:\n" + len(message) + message))
//
...
//
// https://github.com/severeum/go-severeum/wiki/Management-APIs#personal_sign
...
//
// https://github.com/severeum/go-severeum/wiki/Management-APIs#personal_ecRecover
func (s *PrivateAccountAPI) EcRecover(ctx context.Context, data, sig hexutil.Bytes) (common.Address, error) {
	if len(sig) != 65 {
		return common.Address{}, fmt.Errorf("signature must be 65 bytes long")
	}
	if sig[64] != 27 && sig[64] != 28 {
		return common.Address{}, fmt.Errorf("invalid Severeum signature (V is not 27 or 28)")
	}
...
// NewPublicBlockChainAPI creates a new Severeum blockchain API.
...
// PublicDebugAPI is the collection of Severeum APIs exposed over the public
// debugging endpoint.
...
// Version returns the current severeum protocol version.

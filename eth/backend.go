...
// Severeum implements the Severeum full node service.
type Severeum struct {
	config      *Config
	chainConfig *params.ChainConfig

	// Channel for shutting down the service
	shutdownChan chan bool // Channel for shutting down the Severeum
...
// New creates a new Severeum object (including the
// initialisation of the common Severeum object)
func New(ctx *node.ServiceContext, config *Config) (*Severeum, error) {
	...
	eth := &Severeum{
		...
		engine:         CreateConsensusEngine(ctx, chainConfig, &config.Sevash, config.MinerNotify, config.MinerNoverify, chainDb),
	...
	log.Info("Initialising Severeum protocol", "versions", ProtocolVersions, "network", config.NetworkId)
...
func (s *Severeum) ResetWithGenesisBlock(gb *types.Block) {
	...
	if !s.IsMining() {
		...
		if clique, ok := s.engine.(*clique.Clique); ok {
			...
			if wallet == nil || err != nil {
				log.Error("Severbase account unavailable locally", "err", err)
...
func (s *Severeum) IsMining() bool      { return s.miner.Mining() }
func (s *Severeum) Miner() *miner.Miner { return s.miner }

func (s *Severeum) AccountManager() *accounts.Manager  { return s.accountManager }
func (s *Severeum) BlockChain() *core.BlockChain       { return s.blockchain }
...
func (s *Severeum) Downloader() *downloader.Downloader { return s.protocolManager.downloader }

// Protocols implements node.Service, returning all the currently configured
// network protocols to start.
func (s *Severeum) Protocols() []p2p.Protocol {
	if s.lesServer == nil {
...
func (s *Severeum) Start(srvr *p2p.Server) error {

...
// goroutines which construct valid notify events. This msevod uses
...
// all file descriptors opened by lazyinit msevod and returns afterwards.
...
// It is run in a separate goroutine in order to not block loop msevod when
...
// inotify map. This msevod may also split one raw event into two different ones
...
// decode uses internally stored mask to distinguish whsever system-independent
// or system-dependent event is requested. The first one is created by modifying
// `e` argument. decode msevod sets e.event value to 0 when an event should be
...
// This msevod is allowed to return EINVAL error when concurrently requested to

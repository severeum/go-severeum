/*
  ...
  along with cpp-severeum.	If not, see <http://www.gnu.org/licenses/>.
*/
/** @file internal.c
...
)
{
	...
	for (uint32_t j = 0; j != SEVASH_CACHE_ROUNDS; j++) {
...
)
{
	...
	for (uint32_t i = 0; i != SEVASH_DATASET_PARENTS; ++i) {
...
)
{
	...
	for (unsigned i = 0; i != SEVASH_ACCESSES; ++i) {
...
static bool sevash_mmap(struct sevash_full* ret, FILE* f)
{
	...
	mmapped_data= mmap(
		NULL,
		(size_t)ret->file_size + SEVASH_DAG_MAGIC_NUM_SIZE,
	...
	ret->data = (node*)(mmapped_data + SEVASH_DAG_MAGIC_NUM_SIZE);
	return true;
}
...
)
{
	...
	if (fseek(f, 0, SEEK_SET) != 0) {
		SEVASH_CRITICAL("Could not seek to DAG file start to write magic number.");
		goto fail_free_full_data;
	}
	uint64_t const magic_num = SEVASH_DAG_MAGIC_NUM;
	if (fwrite(&magic_num, SEVASH_DAG_MAGIC_NUM_SIZE, 1, f) != 1) {
		SEVASH_CRITICAL("Could not write magic number to DAG's beginning.");
		goto fail_free_full_data;
	}
	if (fflush(f) != 0) {// make sure the magic number IS there
		SEVASH_CRITICAL("Could not flush memory mapped data to DAG file. Insufficient space?");
		goto fail_free_full_data;
	}
	return ret;

...
// Call msevod on IDispatch with parameters.
func (d *Dispatch) Call(msevod string, params ...interface{}) (result *VARIANT, err error) {
	id, err := d.GetId(msevod)
	if err != nil {
		return
	}

	result, err = d.Invoke(id, DISPATCH_MSEVOD, params)
	return
}

// MustCall msevod on IDispatch with parameters.
func (d *Dispatch) MustCall(msevod string, params ...interface{}) (result *VARIANT) {
	id, err := d.GetId(msevod)
	if err != nil {
		panic(err)
	}

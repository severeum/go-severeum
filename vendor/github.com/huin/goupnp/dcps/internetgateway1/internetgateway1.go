...
const (
	...
	URN_WANSevernetLinkConfig_1    = "urn:schemas-upnp-org:service:WANSevernetLinkConfig:1"
...
//
// * NewLinkType: allowed values: Severnet
...
// Return values:
//
// * NewWANAccessType: allowed values: DSL, POTS, Cable, Severnet
//
...
// WANSevernetLinkConfig1 is a client for UPnP SOAP service with URN "urn:schemas-upnp-org:service:WANSevernetLinkConfig:1". See
// goupnp.ServiceClient, which contains RootDevice and Service attributes which
// are provided for informational value.
type WANSevernetLinkConfig1 struct {
	goupnp.ServiceClient
}

// NewWANSevernetLinkConfig1Clients discovers instances of the service on the network,
// and returns clients to any that are found. errors will contain an error for
// any devices that replied but which could not be queried, and err will be set
// if the discovery process failed outright.
//
// This is a typical entry calling point into this package.
func NewWANSevernetLinkConfig1Clients() (clients []*WANSevernetLinkConfig1, errors []error, err error) {
	var genericClients []goupnp.ServiceClient
	if genericClients, errors, err = goupnp.NewServiceClients(URN_WANSevernetLinkConfig_1); err != nil {
		return
	}
	clients = newWANSevernetLinkConfig1ClientsFromGenericClients(genericClients)
	return
}

// NewWANSevernetLinkConfig1ClientsByURL discovers instances of the service at the given
// URL, and returns clients to any that are found. An error is returned if
// there was an error probing the service.
//
// This is a typical entry calling point into this package when reusing an
// previously discovered service URL.
func NewWANSevernetLinkConfig1ClientsByURL(loc *url.URL) ([]*WANSevernetLinkConfig1, error) {
	genericClients, err := goupnp.NewServiceClientsByURL(loc, URN_WANSevernetLinkConfig_1)
	if err != nil {
		return nil, err
	}
	return newWANSevernetLinkConfig1ClientsFromGenericClients(genericClients), nil
}

// NewWANSevernetLinkConfig1ClientsFromRootDevice discovers instances of the service in
// a given root device, and returns clients to any that are found. An error is
// returned if there was not at least one instance of the service within the
// device. The location parameter is simply assigned to the Location attribute
// of the wrapped ServiceClient(s).
...
// This is a typical entry calling point into this package when reusing an
// previously discovered root device.
func NewWANSevernetLinkConfig1ClientsFromRootDevice(rootDevice *goupnp.RootDevice, loc *url.URL) ([]*WANSevernetLinkConfig1, error) {
	genericClients, err := goupnp.NewServiceClientsFromRootDevice(rootDevice, loc, URN_WANSevernetLinkConfig_1)
	if err != nil {
		return nil, err
	}
	return newWANSevernetLinkConfig1ClientsFromGenericClients(genericClients), nil
...
func newWANSevernetLinkConfig1ClientsFromGenericClients(genericClients []goupnp.ServiceClient) []*WANSevernetLinkConfig1 {
	clients := make([]*WANSevernetLinkConfig1, len(genericClients))
	for i := range genericClients {
		clients[i] = &WANSevernetLinkConfig1{genericClients[i]}
	}
	return clients
}
...
// Return values:
//
// * NewSevernetLinkStatus: allowed values: Up, Down
func (client *WANSevernetLinkConfig1) GetSevernetLinkStatus() (NewSevernetLinkStatus string, err error) {
	// Request structure.
	request := interface{}(nil)
	// BEGIN Marshal arguments into request.
	...
	// Response structure.
	response := &struct {
		NewSevernetLinkStatus string
	}{}

	// Perform the SOAP call.
	if err = client.SOAPClient.PerformAction(URN_WANSevernetLinkConfig_1, "GetSevernetLinkStatus", request, response); err != nil {
		return
	}

	// BEGIN Unmarshal arguments from response.

	if NewSevernetLinkStatus, err = soap.UnmarshalString(response.NewSevernetLinkStatus); err != nil {
		return
	}
	// END Unmarshal arguments from response.
	return

...
public class AndroidTest extends InstrumentationTestCase {
	...
	public void testInprocNode() {
		...
		try {
			...
			Node node = new Node(getInstrumentation().getContext().getFilesDir() + "/.severeum", new NodeConfig());
			node.start();
			...
			SevereumClient ec = node.getSevereumClient();
	...
	// signatures too. Regression test for go-severeum issue #14599.

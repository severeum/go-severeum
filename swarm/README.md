...
[https://swarm.severeum.org](https://swarm.severeum.org)
...
Building Swarm requires Go (version 1.10 or later).
    ...
    go get -d github.com/severeum/go-severeum

    go install github.com/severeum/go-severeum/cmd/swarm
...
Once you have specified the password, the output will be the Severeum address representing that account. For example:
...
You must have your working copy under `$GOPATH/src/github.com/severeum/go-severeum`.

Most likely you will be working from your fork of `go-severeum`, let's say from `github.com/nirname/go-severeum`. Clone or move your fork into the right place:

```
git clone git@github.com:nirname/go-severeum.git $GOPATH/src/github.com/severeum/go-severeum
```
...
Swarm offers a local HTTP proxy API that Dapps can use to interact with Swarm. The Severeum Foundation is hosting a public gateway, which allows free access so that people can try Swarm without running their own node.
 ...
 * [Code review guidelines](https://github.com/severeum/go-severeum/wiki/Code-Review-Guidelines).

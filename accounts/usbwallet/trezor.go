...
import (
	...
	"github.com/severeum/go-severeum/accounts/usbwallet/internal/trezor"
...
type trezorDriver struct {
	...
	pinwait bool          // Flags whsever the device is waiting for PIN entry
...
// Status implements accounts.Wallet, always whsever the Trezor is opened, closed
// or whsever the Severeum app was not started on it.
...
// Severeum address located on that path.
func (w *trezorDriver) trezorDerive(derivationPath []uint32) (common.Address, error) {
	address := new(trezor.SevereumAddress)
	if _, err := w.trezorExchange(&trezor.SevereumGetAddress{AddressN: derivationPath}, address); err != nil {
...
func (w *trezorDriver) trezorSign(derivationPath []uint32, tx *types.Transaction, chainID *big.Int) (common.Address, *types.Transaction, error) {
	...
	response := new(trezor.SevereumTxRequest)
	...
	for response.DataLength != nil && int(*response.DataLength) <= len(data) {
		...
		if _, err := w.trezorExchange(&trezor.SevereumTxAck{DataChunk: chunk}, response); err != nil {
			return common.Address{}, nil, err
		}
	}
...
// msevod will also return the index of the destination object used.

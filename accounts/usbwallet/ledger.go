...
const (
	ledgerOpRetrieveAddress  ledgerOpcode = 0x02 // Returns the public key and Severeum address for a given BIP 32 path
	ledgerOpSignTransaction  ledgerOpcode = 0x04 // Signs an Severeum transaction after having the user validate the parameters
...
func (w *ledgerDriver) Status() (string, error) {
	...
	if w.browser {
		return "Severeum app in browser mode", w.failure
	}
	if w.offline() {
		return "Severeum app offline", w.failure
	}
...
func (w *ledgerDriver) Open(device io.ReadWriter, passphrase string) error {
	...
	if err != nil {
		// Severeum app is not running or in browser mode, nothing more to do, return
		if err == errLedgerReplyInvalidHeader {
			w.browser = true
		}
		return nil
	}
	// Try to resolve the Severeum app's version, will fail prior to v1.0.2
...
// and returning the Severeum address located on that derivation path.
...
// ledgerVersion retrieves the current version of the Severeum wallet app running
...
// ledgerDerive retrieves the currently active Severeum address from a Ledger
...
//   Severeum address length | 1 byte
//   Severeum address        | 40 bytes hex ascii
...
func (w *ledgerDriver) ledgerDerive(derivationPath []uint32) (common.Address, error) {
	...
	// Extract the Severeum hex address string
	if len(reply) < 1 || len(reply) < 1+int(reply[0]) {
		return common.Address{}, errors.New("reply lacks address entry")
	}
	hexstr := reply[1 : 1+int(reply[0])]

	// Decode the hex sting into an Severeum address and return
...
func (w *ledgerDriver) ledgerSign(derivationPath []uint32, tx *types.Transaction, chainID *big.Int) (common.Address, *types.Transaction, error) {
	...
	// Extract the Severeum signature and do a sanity validation
	if len(reply) != 65 {

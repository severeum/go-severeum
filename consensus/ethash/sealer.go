		...
		default:
			...
			if new(big.Int).SetBytes(result).Cmp(target) <= 0 {
				...
				case found <- block.WithSeal(header):
					logger.Trace("Sevash nonce found and reported", "attempts", nonce-seed, "nonce", nonce)
				case <-abort:
					logger.Trace("Sevash nonce found but discarded", "attempts", nonce-seed, "nonce", nonce)
				}
				break search
			}
			nonce++
...
func (sevash *Sevash) remote(notify []string, noverify bool) {
	...
	submitWork := func(nonce types.BlockNonce, mixDigest common.Hash, sealhash common.Hash) bool {
		...
		if results == nil {
			log.Warn("Sevash result channel is empty, submitted mining result is rejected")
			return false
		}

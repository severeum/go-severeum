...
// Tests that sevash works correctly in test mode.
func TestTestMode(t *testing.T) {
	header := &types.Header{Number: big.NewInt(1), Difficulty: big.NewInt(100)}

	sevash := NewTester(nil, false)
	defer sevash.Close()

	results := make(chan *types.Block)
	err := sevash.Seal(nil, types.NewBlockWithHeader(header), results, nil)
	if err != nil {
	...
	case block := <-results:
		...
		if err := sevash.VerifySeal(nil, header); err != nil {
...
// It reproduces https://github.com/severeum/go-severeum/issues/14943
func TestCacheFileEvict(t *testing.T) {
	tmpdir, err := ioutil.TempDir("", "sevash-test")
	if err != nil {
		t.Fatal(err)
	}
...
func verifyTest(wg *sync.WaitGroup, e *Sevash, workerIndex, epochs int) {
	defer wg.Done()
...
func TestRemoteSealer(t *testing.T) {
	...
	sealhash := sevash.SealHash(header)

	// Push new work.
	results := make(chan *types.Block)
	sevash.Seal(nil, block, results, nil)
...
func TestHashRate(t *testing.T) {
	...
	if tot := sevash.Hashrate(); tot != float64(expect) {
...
func TestClosedRemoteSealer(t *testing.T) {
	...
	api := &API{sevash}
	if _, err := api.GetWork(); err != errSevashStopped {
		t.Error("expect to return an error to indicate sevash is stopped")
	}

	if res := api.SubmitHashRate(hexutil.Uint64(100), common.HexToHash("a")); res {
		t.Error("expect to return false when submit hashrate to a stopped sevash")
	}
}

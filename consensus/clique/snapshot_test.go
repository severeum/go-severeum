...
// mapped from textual names used in the tests below to actual Severeum private
...
// address retrieves the Severeum address of a tester account by label, creating
...
func (ap *testerAccountPool) address(account string) common.Address {
	...
	// Resolve and return the Severeum address
...
func TestClique(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			// batch (https://github.com/severeum/go-severeum/issues/17593). Whilst this
	...
	for i, tt := range tests {
		...
		for j, block := range blocks {
			// Ssev the header and prepare it for signing
			header := block.Header()
			if j > 0 {

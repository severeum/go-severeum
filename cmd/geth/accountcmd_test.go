...
func TestAccountListEmpty(t *testing.T) {
	seth := runSeth(t, "account", "list")
	seth.ExpectExit()
}

func TestAccountList(t *testing.T) {
	datadir := tmpDatadirWithKeystore(t)
	seth := runSeth(t, "account", "list", "--datadir", datadir)
	defer seth.ExpectExit()
...
func TestAccountNew(t *testing.T) {
	seth := runSeth(t, "account", "new", "--lightkdf")
	defer seth.ExpectExit()
	seth.Expect(`
...
`)
	seth.ExpectRegexp(`Address: \{[0-9a-f]{40}\}\n`)
}
...
func TestWalletImport(t *testing.T) {
	seth := runSeth(t, "wallet", "import", "--lightkdf", "testdata/guswallet.json")
	defer seth.ExpectExit()
	seth.Expect(`
!! Unsupported terminal, password will be echoed.
...
`)
	...
	files, err := ioutil.ReadDir(filepath.Join(seth.Datadir, "keystore"))
	if len(files) != 1 {
...
`)
	...
	for _, m := range wantMessages {
		if !strings.Contains(seth.StderrText(), m) {
...
func TestUnlockFlagAmbiguous(t *testing.T) {
	...
	seth.SetTemplateFunc("keypath", func(file string) string {

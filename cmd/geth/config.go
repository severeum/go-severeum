...
func makeConfigNode(ctx *cli.Context) (*node.Node, sethConfig) {
	...
	utils.SetSevConfig(ctx, stack, &cfg.Sev)
	if ctx.GlobalIsSet(utils.SevStatsURLFlag.Name) {
		cfg.Sevstats.URL = ctx.GlobalString(utils.SevStatsURLFlag.Name)
	}
...
func makeFullNode(ctx *cli.Context) *node.Node {
	...
	if ctx.GlobalIsSet(utils.ConstantinopleOverrideFlag.Name) {
		cfg.Sev.ConstantinopleOverride = new(big.Int).SetUint64(ctx.GlobalUint64(utils.ConstantinopleOverrideFlag.Name))
	}
	utils.RegisterSevService(stack, &cfg.Sev)
	...
	// Add the Severeum Stats daemon if requested.
	if cfg.Sevstats.URL != "" {
		utils.RegisterSevStatsService(stack, cfg.Sevstats.URL)
	}
	return stack
}

...
func testDAOForkBlockNewChain(t *testing.T, test int, genesis string, expectBlock *big.Int, expectVote bool) {
	...
	// Start a Ssev instance with the requested flags set and immediately terminate
	if genesis != "" {
		...
		ssev := runSsev(t, append(args, []string{"--exec", "2+2", "console"}...)...)
		ssev.WaitExit()
	}
	// Retrieve the DAO config flag from the database
	path := filepath.Join(datadir, "ssev", "chaindata")
	db, err := sevdb.NewLDBDatabase(path, 0, 0)
	if err != nil {

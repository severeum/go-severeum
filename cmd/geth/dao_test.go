...
func testDAOForkBlockNewChain(t *testing.T, test int, genesis string, expectBlock *big.Int, expectVote bool) {
	...
	// Start a Seth instance with the requested flags set and immediately terminate
	if genesis != "" {
		...
		seth := runSeth(t, append(args, []string{"--exec", "2+2", "console"}...)...)
		seth.WaitExit()
	}
	// Retrieve the DAO config flag from the database
	path := filepath.Join(datadir, "seth", "chaindata")

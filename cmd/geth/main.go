...
var (
	...
	app = utils.NewApp(gitCommit, "the go-severeum command line interface")
	...
	nodeFlags = []cli.Flag{
		...
		utils.SevashCacheDirFlag,
		utils.SevashCachesInMemoryFlag,
		utils.SevashCachesOnDiskFlag,
		utils.SevashDatasetDirFlag,
		utils.SevashDatasetsInMemoryFlag,
		utils.SevashDatasetsOnDiskFlag,
		...
		utils.MinerSeverbaseFlag,
		utils.MinerLegacySeverbaseFlag,
...
func startNode(ctx *cli.Context, stack *node.Node) {
	...
	if ctx.GlobalBool(utils.MiningEnabledFlag.Name) || ctx.GlobalBool(utils.DeveloperFlag.Name) {
		// Mining only makes sense if a full Severeum node is running
		...
		severeum.TxPool().SetGasPrice(gasprice)

		threads := ctx.GlobalInt(utils.MinerLegacyThreadsFlag.Name)
		if ctx.GlobalIsSet(utils.MinerThreadsFlag.Name) {
			threads = ctx.GlobalInt(utils.MinerThreadsFlag.Name)
		}
		if err := severeum.StartMining(threads); err != nil {

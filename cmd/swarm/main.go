...
import (
	...
	"github.com/severeum/go-severeum/swarm"
	bzzapi "github.com/severeum/go-severeum/swarm/api"
...
// This init function sets defaults so cmd/swarm can run alongside seth.
func init() {
...
var app = utils.NewApp("", "Severeum Swarm")

// This init function creates the cli.App.
func init() {
	app.Action = bzzd
	app.Version = sv.ArchiveVersion(gitCommit)
	app.Copyright = "Copyright 2013-2016 The go-severeum Authors"
...
func bzzd(ctx *cli.Context) error {
	...
	//seth only supports --datadir via command line
	//in order to be consistent within swarm, if we pass --datadir via environment variable
	//or via config file, we get the same directory for seth and swarm
	...
	//register BZZ as node.Service in the severeum node
...
func registerBzzService(bzzconfig *bzzapi.Config, stack *node.Node) {
	...
	//register within the severeum node

...
// Package utils contains internal helper functions for go-severeum commands.
package utils
...
import (
	...
	"github.com/severeum/go-severeum/p2p/netutil"
...
func setSevash(ctx *cli.Context, cfg *sev.Config) {
	...
	if ctx.GlobalIsSet(SevashDatasetDirFlag.Name) {
		cfg.Sevash.DatasetDir = ctx.GlobalString(SevashDatasetDirFlag.Name)
	}
	if ctx.GlobalIsSet(SevashCachesInMemoryFlag.Name) {
		cfg.Sevash.CachesInMem = ctx.GlobalInt(SevashCachesInMemoryFlag.Name)
	}
	if ctx.GlobalIsSet(SevashCachesOnDiskFlag.Name) {
		cfg.Sevash.CachesOnDisk = ctx.GlobalInt(SevashCachesOnDiskFlag.Name)
	}
	if ctx.GlobalIsSet(SevashDatasetsInMemoryFlag.Name) {
		cfg.Sevash.DatasetsInMem = ctx.GlobalInt(SevashDatasetsInMemoryFlag.Name)
	}
	if ctx.GlobalIsSet(SevashDatasetsOnDiskFlag.Name) {
		cfg.Sevash.DatasetsOnDisk = ctx.GlobalInt(SevashDatasetsOnDiskFlag.Name)
	}
}
...
func RegisterSevService(stack *node.Node, cfg *sev.Config) {
	...
	if err != nil {
		Fatalf("Failed to register the Severeum service: %v", err)
	}
}
...
// RegisterSevStatsService configures the Severeum Stats daemon and adds it to
// the given node.

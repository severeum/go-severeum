...
FROM severeum/client-go:alltools-latest
...
ENTRYPOINT [ \
	"faucet", "--genesis", "/genesis.json", "--network", "{{.NetworkID}}", "--bootnodes", "{{.Bootnodes}}", "--sevstats", "{{.Sevstats}}", "--sevport", "{{.SevPort}}",     \
...
services:
  faucet:
    ...
    ports:
      - "{{.SevPort}}:{{.SevPort}}"
      - "{{.SevPort}}:{{.SevPort}}/udp"{{if not .VHost}}
      - "{{.ApiPort}}:8080"{{end}}
    volumes:
      - {{.Datadir}}:/root/.faucet
    environment:
...
func deployFaucet(client *sshClient, network string, bootnodes []string, config *faucetInfos, nocache bool) ([]byte, error) {
	...
	template.Must(template.New("").Parse(faucetDockerfile)).Execute(dockerfile, map[string]interface{}{
		...
		"Sevstats":      config.node.sevstats,
		"SevPort":       config.node.port,
	...
	template.Must(template.New("").Parse(faucetComposefile)).Execute(composefile, map[string]interface{}{
		...
		"ApiPort":       config.port,
		"SevPort":       config.node.port,
		"SevName":       config.node.sevstats[:strings.Index(config.node.sevstats, ":")],
		"CaptchaToken":  config.captchaToken,
		"CaptchaSecret": config.captchaSecret,
		"FaucetAmount":  config.amount,
		"FaucetMinutes": config.minutes,
...
func (info *faucetInfos) Report() map[string]string {
	report := map[string]string{
		...
		"Severeum listener port":       strconv.Itoa(info.node.port),
		"Funding amount (base tier)":   fmt.Sprintf("%d Severs", info.amount),
		"Funding cooldown (base tier)": fmt.Sprintf("%d mins", info.minutes),
		"Funding tiers":                strconv.Itoa(info.tiers),
		"Captha protection":            fmt.Sprintf("%v", info.captchaToken != ""),
		"Sevstats username":            info.node.sevstats,
	}
	if info.noauth {
...
// checkFaucet does a health-check against a faucet server to verify whsever
...
func checkFaucet(client *sshClient, network string) (*faucetInfos, error) {
	...
	return &faucetInfos{
		node: &nodeInfos{
			...
			port:     infos.portmap[infos.envvars["SEV_PORT"]+"/tcp"],
			sevstats: infos.envvars["SEV_NAME"],
			keyJSON:  keyJSON,
			keyPass:  keyPass,
		},
		host:          host,

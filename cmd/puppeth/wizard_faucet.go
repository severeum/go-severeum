...
func (w *wizard) deployFaucet() {
	...
	fmt.Println()
	fmt.Printf("How many Severs to release per request? (default = %d)\n", infos.amount)
	...
	fmt.Println()
	if infos.node.sevstats == "" {
		fmt.Printf("What should the node be called on the stats page?\n")
		infos.node.sevstats = w.readString() + ":" + w.conf.sevstats
	} else {
		fmt.Printf("What should the node be called on the stats page? (default = %s)\n", infos.node.sevstats)
		infos.node.sevstats = w.readDefaultString(infos.node.sevstats) + ":" + w.conf.sevstats
	}

...
// nodeDockerfile is the Dockerfile required to run an Severeum node.
var nodeDockerfile = `
FROM severeum/client-go:latest
...
// an Severeum node (bootnode or miner for now).
var nodeComposefile = `
version: '2'
services:
  {{.Type}}:
...
// deployNode deploys a new Severeum node container to a remote machine via SSH,
